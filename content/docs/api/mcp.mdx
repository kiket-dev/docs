...
- no legacy snake_case DTO assumptions
...
- **Cases** (MCP tools may still use legacy `*_issue*` identifiers until the MCP package aligns names with the REST contract): `list_issues`, `get_issue`, `create_issue`, `get_issue_reachable_transitions`, `check_issue_transition`, `transition_issue`, `get_issue_history`
- comments: `list_issue_comments`, `add_issue_comment`
- planning metadata: `list_milestones`, `list_issue_types` (**case-type** listing; legacy tool identifier)
