...
- 🔌 **Webhook decorators** – define handlers with `sdk.webhook("case.created", version: "v1")`.
...
sdk.register('case.created', version: 'v1') do |payload, context|
  summary = payload['case']['title']
  puts "Event version: #{context[:event_version]}"

  context[:endpoints].log_event('case.created', summary: summary)
...
sdk.register('case.created', version: 'v2') do |payload, context|
  summary = payload['case']['title']

  context[:endpoints].log_event('case.created', summary: summary, schema: 'v2')
...
```ruby
sdk.register('case.created', version: 'v1') do |payload, context|
  project_id = payload.dig('case', 'project_id')
...
sdk.register('case.created', version: 'v1', required_scopes: ['cases.read', 'cases.write']) do |payload, context|
  # Handler only executes if scopes are present
  context[:endpoints].log_event('case.processed', { id: payload['case']['id'] })
  { ok: true }
end
...
extension:
  output_fields:
    inbound_email:
      label: Inbound Email
      description: Send emails to this address to create cases
      type: copyable
      icon: envelope
    connection_status:
...
RSpec.describe 'My webhook handler' do
  ...
  it 'handles case.created event' do
    sdk.register('case.created', version: 'v1') do |payload, context|
      { processed: payload['case']['id'] }
    end

    payload = { case: { id: '123', title: 'Test Issue' } }
