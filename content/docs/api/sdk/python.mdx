...
@sdk.webhook("case.created", version="v1")
async def handle_issue(payload, context):
    summary = payload["case"]["title"]
    assert context.event_version == "v1"
    await context.endpoints.log_event("case.created", summary=summary)
    await context.secrets.set("WEBHOOK_TOKEN", "abc123")
    return {"ok": True}

@sdk.webhook("case.created", version="v2")
async def handle_issue_v2(payload, context):
    summary = payload["case"]["title"]
    await context.endpoints.log_event("case.created", summary=summary, schema="v2")
...
async def handle_sla(payload, context):
    project_id = payload["case"]["project_id"]
...
@sdk.webhook("case.created", version="v1", required_scopes=["cases.read", "cases.write"])
async def handle_issue(payload, context):
    # Handler only executes if scopes are present
    await context.endpoints.log_event("case.processed", id=payload["case"]["id"])
    return {"ok": True}
