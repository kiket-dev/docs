...
- 🔌 **Webhook decorators** – define handlers with `sdk.webhook("case.created", "v1")`.
...
- 📇 **Custom data helper** – call `/api/v1/ext/custom_data/...` with `context.endpoints.customData(projectId)` using the runtime token.
...
sdk.webhook('case.created', 'v1')((payload, context) => {
  const summary = payload.case.title;
  console.log(`Event version: ${context.eventVersion}`);

  await context.endpoints.logEvent('case.created', { summary });
...
sdk.webhook('case.created', 'v2')(async (payload, context) => {
  const summary = payload.case.title;

  await context.endpoints.logEvent('case.created', {
    summary,
    schema: 'v2'
  });
...
When your manifest declares `custom_data.permissions`, the SDK automatically uses the runtime token provided in the webhook payload for API calls via `context.client`:

```typescript
sdk.webhook('case.created', 'v1')(async (payload, context) => {
  const projectId = payload.case.project_id;
...
describe('My webhook handler', () => {
  it('should handle case.created event', async () => {
    ...
    const handler = sdk.webhook('case.created', 'v1')((payload, context) => {
      return { processed: payload.case.id };
    });

    // Create signed payload for testing
    const { body, headers } = createSignedPayload('test-secret', {
      case: { id: '123', title: 'Test Issue' }
    });

    // Test the handler
    const response = await request(sdk.app)
      .post('/v/1/webhooks/case.created')
      .set(headers)
      .send(body);
...
Create an `extension.yaml` or `manifest.yaml` file in your workspace root:
...
sdk.register('case.created', handler, 'v1', ['cases.read', 'cases.write']);

// Or using the decorator
sdk.webhook('case.created', 'v1', ['cases.read', 'cases.write'])(async (payload, context) => {
  // Handler only executes if scopes are present
  await context.endpoints.logEvent('case.processed', { id: payload.case.id });
  return { ok: true };
});
