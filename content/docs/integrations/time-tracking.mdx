---
title: Time Tracking
description: Time Tracking integration for Kiket.
---
...
Automatically handles operational case transitions. When a case moves to `closed` or `done`, all active timers linked to that case id are stopped and tagged with `"auto-stopped"`.

Handlers are also registered under the legacy webhook name `time_tracking.webhooks.issue_transitioned`.
...
Legacy payloads may nest the id under `issue` instead of `case`; both are accepted.
...
- Check webhook is configured for `case.transitioned` (legacy: `issue.transitioned`) when using platform event synonyms
- Verify webhook payload includes `transition.to` (`closed` or `done`) and `case.id` or `issue.id`
