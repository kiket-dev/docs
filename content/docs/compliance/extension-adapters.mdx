...
For custom external runners, set `KIKET_EXTENSION_RUNNER_API_KEY` to that key and `KIKET_PLATFORM_BASE_URL` in the runner process. Catalog-hosted first-party extensions normalize in-process on the API (no separate runner). See [extension runtime registry](../architecture/extension-source-registry.md).

For customer-owned extensions and the public registry, see [extension runtime registry (uniform Model A)](../architecture/extension-source-registry.md).
