...
`kiket extension init` writes `kiket-extension.yaml` and `src/adapter.ts`. Normalization for shipped first-party adapters also lives in the monorepo (`packages/github-adapter`, `packages/slack-adapter`, `packages/webhook-adapter`, `packages/jira-adapter`); extension submodules re-export those packages for packaging and tests.
